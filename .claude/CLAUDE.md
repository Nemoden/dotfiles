...
- **The `X is Y, not Z` construct earns its place or goes.** Keep it only when a reader would plausibly have picked Z: `service must be "matters", not "adieu-pool-matters-v001"` names a real wrong answer. Cut it when Z is nobody's guess (`the note is prose, not a schema`) or when the pair is an aphorism dressed as insight (`the asymmetry is about identity, not trust`). Test before writing it: name the reader who would have believed Z. If you cannot, state the positive claim and stop. One per section at most, and never two in a paragraph. This applies to the same shape spelled other ways: `not Z but Y`, `Y rather than Z`, `Z? No. Y.`
  - **Exception: Z is on the record.** When someone actually asserted Z — the user, a reviewer, a ticket, an earlier answer of yours — the construct is the right tool, because correcting a stated claim needs the claim named. `expl222 is for self-education, not myth-busting` earns it when a previous turn treated myth-busting as the entry requirement. The test above still governs and this exception passes it: the reader who believed Z is whoever said Z. Quote or paraphrase Z close enough that a cold reader sees which claim is being retired, and drop the per-section cap for a correction, because the correction is the point. This does not license the aphorism form: an invented Z nobody advanced is still out.
