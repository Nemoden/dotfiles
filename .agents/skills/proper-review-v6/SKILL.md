---
name: proper-review-v6
description: Use when reviewing a PR, diff, branch, or any proposed code change. Same staged senior-engineer review as v5 (cold read → stated WHY → reconcile with HARD HALT → calibrate → blind independent solve → compare → reach sweep → bots LAST), but the full review prose is written to a /tmp file and only a distilled emoji statusline TL;DR is surfaced to chat — the full review also stays in the agent's context, so "expand N" / "show the WHY chain" answers instantly with the file as a durable fallback. Quality-of-life: read the headline, drill in on demand. Halts and clarifying asks always surface fully in chat. Triggers on "review this PR", "proper review v6", "review the diff", "/proper-review-v6", a GitHub PR URL, or any code review request.
---

# Proper Review v6

**v6 = v5 stage discipline, distilled output.** Every stage gate, every probe, the cold-read → reconcile → blind-solve → reach-sweep → bots-last order — identical to v5. The ONLY difference is *delivery*: the full review prose is written to a `/tmp` file, the full review is also held in the agent's working context, and **only a distilled emoji statusline lands in chat**. You read the headline; you ask to expand anything; the answer comes from context (file is the durable fallback if context is later compacted or you want to read it yourself).
...
## How v6 delivers output (read this first)
...
The stage **Output:** blocks below define what goes *into the file*, not what streams to chat. In v6, "Output:" = "write this section to the review document."
...
- **Verdict** (always): `🟢 SHIP` · `🟢 SHIP-WITH-NITS` · `🟡 TRIM` · `🟧 RESCOPE` · `🟥 KILL`. The light leads the strip.
...
````
# Review: <title> — <slug>
_Generated by proper-review-v6. Verdict: <VERDICT>. Full prose; chat showed only the statusline._
...
- **Streaming stage prose to chat.** v6 runs Stages 1a–5 silently; the chat surface is the Stage-6 statusline only. The exceptions are halts and clarifying asks (2.7 mismatch, 2 stop/ask, 1.5 tier ask, 2.5 problem-statement checkpoint) — those surface fully inline.
